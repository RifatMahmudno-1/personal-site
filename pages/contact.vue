	...
	<div class="flex-grow grid justify-items-center items-center">
		<form class="w-full max-w-[40rem] bg-cyan-300 shadow-md rounded p-4 flex flex-col gap-4" @submit.prevent="submit">
			...
			<div class="grid items-center gap-2 grid-cols-[auto_1fr]">
				...
				<input type="email" name="email" id="email" required placeholder="Enter your email" minlength="8" maxlength="256" class="bg-white w-full rounded px-1 focus:shadow-md transition-shadow" v-model="body.email" :disabled="sending" />
				<label for="message" class="self-start">Message:</label>
				<textarea name="message" id="message" required placeholder="Enter message here" maxlength="5000" class="bg-white rounded px-1 min-h-[5rem] resize-y focus:shadow-md transition-shadow" v-model="body.message" :disabled="sending"></textarea>
			</div>
...
<script setup lang="ts">
	...
	async function submit() {
		sending.value = true
		try {
			await $fetch('/api/contact', { method: 'POST', body: body.value })
