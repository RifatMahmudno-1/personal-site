...
<script setup lang="ts">
	...
	const { data, pending, error } = await cLazyFetch<EachPostType[]>('/api/posts', { responseType: 'json', query: { section } })
	...
	function deleteData(_id: EachPostType['_id']) {
		...
		refreshSidebar!.value = true
		if (!data.value.length) router.push('/')
	}
</script>
